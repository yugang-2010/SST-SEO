function [Ts SEO omega2] = SST_SEO_Y(x,hlength);
...
omega(:,b) = (ft)'+real(va*1i*tfr2(ft,b)/2/pi./tfr1(ft,b));
end 
...
for b=1:nb%time
    ...
    for eta=1:neta%frequency
        if abs(tfr1(eta,b))>0.001*E%you can set much lower value than this.
         
            %omega2(eta,b)=omega(eta,b);
            
            %if abs(omega(eta,b)-eta)<0.5%default frequency resolution is 1Hz.
            if abs(omega(eta,b)-eta)<0.5
            SEO(eta,b)=1;
            end
